...
function ProductWindow({ onStart }) {
  ...
  return <div className={styles.productStage}>
    <div className={styles.productWindow}>
      <div className={styles.productTop}><span className={styles.productMonogram}><Icon name="book" size={18}/></span><strong>Your study mate</strong><span className={styles.demoTag}>Preview</span></div>
      <div className={styles.productGreeting}><small>A LITTLE PROGRESS, EVERY DAY</small><h2>Make today count toward exam day.</h2><p>Read. Revise. Practise. Get exam-ready.</p></div>
      ...
      <div className={styles.productContent}>
        {tab==='Learn'&&<><div className={`${styles.previewCourseArt} ${styles.mint}`}><Icon name="cpu" size={62}/><span>CPE 301</span></div><div className={styles.previewCourseBody}><small>COMPUTER ARCHITECTURE</small><h3>Memory organisation</h3><p>Materials supplied by our team, organised for your course.</p><button type="button" onClick={onStart}>Build my course space <Icon name="arrow" size={17}/></button></div></>}
    ...
    <p className={styles.stageCaption}><Icon name="check" size={17}/> Built to help you get exam-ready.</p>
  </div>;
}
  ...
  return <div className={styles.app} data-theme={theme}>
    ...
    {screen==='landing'?<main id="entry-main" className={styles.landing}>
      <section className={styles.hero} aria-labelledby="landing-title"><div className={styles.heroCopy}>
        <p className={styles.heroBadge}><Icon name="campus" size={17}/> Built for Nigerian tertiary system</p>
        <h1 id="landing-title" tabIndex={-1} ref={titleRef}>Your Entire <span>Kampus</span><br/>in One Place</h1>
        <p className={styles.heroSupport}>Your study mate. Course materials supplied by our academic team, past questions and CBT practice — all organised to help you understand your courses and get exam-ready. No uploads needed.</p>
      ...
      <section className={styles.studentImages} aria-label="Learning on campus and at your own pace">
        <div className={styles.studentPortraitCard}><img src={`${assetBase}/student-on-campus.jpg`} width="625" height="350" alt="A smiling student wearing glasses and a backpack on campus"/><div className={styles.studentAccent}><Icon name="spark" size={30}/><span>Your study<br/>mate.</span><Icon name="arrow" size={23}/></div><p>Study with purpose.<strong>Build understanding. Walk into exams with confidence.</strong></p></div>
        <figure className={styles.studentStudyCard}><img src={`${assetBase}/student-study-space.png`} width="1692" height="929" alt="A student using KampusLearn on a phone beside lecture notes and a laptop" decoding="async"/><figcaption><span>FROM YOUR FIRST LECTURE TO YOUR NEXT EXAM</span><strong>Your study mate, wherever you study.</strong></figcaption></figure>
      </section>

      <section className={styles.toolsSection} id="study-tools" aria-labelledby="tools-title"><div className={styles.sectionIntro}><p className={styles.eyebrow}>LESS SEARCHING. MORE EXAM PREPARATION.</p><h2 id="tools-title">Everything you need<br/>to get exam-ready.</h2><p>Start with your course materials, revise with past questions, then put your understanding to the test.</p></div>
        <div className={styles.featureGrid}>
          <article className={styles.feature}><span className={`${styles.featureIcon} ${styles.mint}`}><Icon name="book" size={25}/></span><small>01 / LEARN</small><h3>Your materials, already organised.</h3><p>Our admins supply and organise course materials and past questions by institution, department and course. You choose what to study — no uploading required.</p><div className={styles.featureTags}><span>Course materials</span><span>Past questions</span></div></article>
          <article className={styles.feature}><span className={`${styles.featureIcon} ${styles.blue}`}><Icon name="check" size={25}/></span><small>02 / PRACTICE</small><h3>Turn revision into confidence.</h3><p>Practise timed CBT tests, review your answers and find the courses that need more revision before exam day.</p><div className={styles.featureTags}><span>CBT practice</span><span>Course progress</span></div></article>
          <article className={styles.feature}><span className={`${styles.featureIcon} ${styles.lilac}`}><Icon name="spark" size={25}/></span><small>03 / AI</small><h3>A study mate for difficult ideas.</h3><p>We’re building AI study support around the course materials our academic team provides, to help make difficult concepts clearer for revision.</p><div className={styles.featureTags}><span>Course-focused support</span><span>Study support</span></div></article>
        </div>
      </section>

      <section className={styles.howSection} id="how-it-works" aria-labelledby="how-title"><div><p className={styles.eyebrow}>MAKE IT YOURS</p><h2 id="how-title">Your route to<br/>exam readiness.</h2><p>Choose your academic profile and courses. We handle the materials; you focus on understanding and practice.</p><ol className={styles.howList}>{[['01','Start with your campus','Tell us your institution, department and level.'],['02','Choose your focus','Select your courses and exam preparation goals. No files to upload.'],['03','Find your rhythm','Work through the supplied materials and past questions, then check your readiness with CBT practice.']].map(([n,title,text])=><li key={n}><span>{n}</span><div><h3>{title}</h3><p>{text}</p></div></li>)}</ol><button type="button" className={styles.primary} onClick={()=>openAuth('signup')}>Start Learning <Icon name="arrow"/></button></div><ProductWindow onStart={()=>openAuth('signup')}/></section>
      ...
      <section className={styles.faqSection} id="questions" aria-labelledby="faq-title"><div><p className={styles.eyebrow}>BEFORE YOU BEGIN</p><h2 id="faq-title">A few things<br/>you might be wondering.</h2></div><div className={styles.faqList}>{[
        ['Who is KampusLearn for?','Students across the Nigerian tertiary system who want organised course materials and focused practice to get exam-ready.'],
        ['Do I need to upload my own materials?','No. KampusLearn admins supply the course materials and past questions. Choose your institution and courses to find what is available for you.'],
        ...
        ['Can I use it for more than exam revision?','Yes. Build your understanding throughout the semester with the materials in Learn, then use past questions and Practice to prepare for exams.'],
        ...
        ...(onAccountNavigate ? [] : [['Does this preview create an account?','No. This preview lets you try the design and onboarding flow. Your choices stay in this page session, and the dashboard uses sample data.']])
      ].map(([q,a])=><details key={q}><summary>{q}<Icon name="plus" size={18}/></summary><p>{a}</p></details>)}</div></section>

      <section className={styles.institutionSection} aria-labelledby="institutions-title"><div className={styles.institutionHeading}><div><p className={styles.eyebrow}>YOUR CAMPUS. YOUR STUDY MATE.</p><h2 id="institutions-title">Built for students across Nigeria.</h2><p>Built around the courses and exam preparation needs of Nigerian tertiary students.</p></div></div><div className={styles.institutionViewport}><div className={styles.institutionTrack}>
      ...
      <footer className={styles.landingFooter}><Brand onClick={()=>{window.scrollTo({top:0,behavior:'auto'});titleRef.current?.focus({preventScroll:true});}}/><p>Your study mate.</p>{!onAccountNavigate && <small>Design preview · No account is created</small>}</footer>
