...
].map(([label,value]) => <div key={label}><dt>{label}</dt><dd>{value || 'Not recorded'}</dd></div>)}</dl><div className={styles.actionRow}><Link className={styles.secondaryButton} href="/onboarding">Edit study setup <Icon name="arrow" size={17}/></Link><button className={styles.secondaryButton} onClick={logout}>Log out</button></div></section>;
}
...
export default function StudentDashboard({ view = 'Home' }) {
  ...
  return <StudentShell>
    {view === 'Home' && <>
      <section className={styles.greeting}><div className={styles.greetingCopy}><p className={styles.date}>{date}</p><h1>{greeting}, {name} 👋</h1><p className={styles.subtitle}>Ready to make progress today?</p><div className={styles.identity}>{[
  profile?.institution_type === 'polytechnic'
    ? profile?.programme_name
    : profile?.department_name,
  profile?.level && `${profile.level}${/^\d/.test(profile.level) ? ' Level' : ''}`,
  profile?.institution_short
].filter(Boolean).map(item => <span key={item}>{item}</span>)}</div></div><Link href="/tokens" className={styles.walletChip} aria-label={wallet.data ? `Wallet: ${wallet.data.balance} KP` : 'Open KP wallet'}><span className={styles.walletGlyph}><Icon name="wallet"/></span><span className={styles.walletInfo}><span className={styles.walletCaption}>KampusCoin</span><strong>{wallet.data ? Number(wallet.data.balance).toLocaleString() : '—'} <small>KP</small></strong><span className={styles.walletCaption}>Open wallet</span></span><Icon name="arrow" size={17}/></Link></section>
