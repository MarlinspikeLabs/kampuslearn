...
export default function Onboarding() {
        ...
        <section className={styles.formPane}><p className={styles.eyebrow}>YOUR SPACE, YOUR PACE</p><h1 ref={heading} tabIndex={-1}>{HEADINGS[step]}</h1><p className={styles.stepSupport}>{step === 0 ? 'Let’s make KampusLearn feel like your learning space.' : step === 1 ? 'These are the academic details you registered with.' : step === 5 ? 'Review your choices, then start learning.' : 'Choose what fits you. You can change this later from your profile.'}</p>
          <form onSubmit={e => { e.preventDefault(); save(Math.min(step + 1, 5), step === 5 ? 'complete' : undefined); }} aria-busy={busy}>
            <fieldset disabled={busy} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}>
              ...
              {step === 1 && <div className={styles.readySummary}><dl>{[
  ...
  ['Level',profile?.level]
].map(([label,value]) => <div key={label}><dt>{label}</dt><dd>{value || 'Not recorded'}</dd></div>)}</dl></div>}
  ...
  : 'Your department’s courses haven’t been added for this level yet. You can continue and choose courses later.'}</p>}{courses.length > 0 && !courses.some(c => `${c.code} ${c.title}`.toLowerCase().includes(search.toLowerCase())) && <p className={styles.emptyCourses}>No courses match that search.</p>}</div>}
        ...
        <aside className={styles.studyAside}><div className={styles.asideTop}><Icon name={['spark','campus','target','book','clock','check'][step]}/><span>MADE FOR YOUR NEXT CHAPTER</span></div><h2>A little structure.<br/>A clearer start.</h2><div className={styles.planCard}><div className={styles.planTitle}><Icon name="home"/><strong>{settings.preferred_name || user.full_name?.split(' ')[0]}’s study space</strong></div><div className={styles.planCampus}>
  <small>{profile?.institution_short || profile?.institution_name}</small>
  <strong>
    {profile?.institution_type === 'polytechnic'
      ? profile?.programme_name
      : profile?.department_name}
  </strong>
</div><div className={styles.planCounts}><div><strong>{selected.length}</strong><span>Courses</span></div><div><strong>{settings.goals.length}</strong><span>Goals</span></div><div><strong>{settings.daily_minutes}m</strong><span>Daily plan</span></div></div></div><p className={styles.asideNote}>Your choices are saved to your account when you continue, so you can pick up on another device.</p></aside>
      </div>
    </main>
  </div>;
}
