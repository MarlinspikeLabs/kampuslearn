...
OUT = BASE / "nbte-2025-polytechnic-programmes-v5.json"
AUDIT = BASE / "nbte-2025-polytechnic-programmes-v5-audit.json"
...
def bad_heading(s):
    ...
    # Programme rows must never become institution boundaries.
    if re.match(
        r"^\s*\d*\s*[.,/]?\s*(?:ND|HND|H\s+ND|HN\s+ND|P-HND)\b",
        s,
        re.I
    ):
        return True

    # Accreditation/table rows are never institution names.
    if re.search(
        r"\b(?:Accredited|Interim|Expired)\b",
        s,
        re.I
    ):
        return True
    ...
    # Institution headings in this directory are uppercase.
    # Reject mixed-case body rows.
...
def credible_boundary(c):
    h = clean(c["heading"])

    # Reject obvious continuation/location fragments.
    if len(h.split()) <= 2:
        return False
    ...
    # Institution-like headings normally contain one
    # of these structural words.
    institution_words = (
        "POLYTECHNIC",
        "COLLEGE",
        "INSTITUTE",
...
def is_table_header_fragment(line):
    """
    Reject repeated PDF table-header fragments that can appear
    immediately after a programme row at page boundaries.
    """
    x = clean(line).lower()
    ...
    fragments = (
        ...
        "status stream date",
        "programmes year granted"
    )

    if any(f in x for f in fragments):
        return True

    # Common broken header fragments.
    words = set(re.findall(r"[a-z]+", x))
        ...
        if last:
            if (
                ...
                if (
                    ...
                    not in line.lower()
                    and not is_table_header_fragment(line)
                    and not (
                        line == line.upper()
                        and len(re.sub(r"[^A-Z]", "", line)) >= 4
                    )
                ):
