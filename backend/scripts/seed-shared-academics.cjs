...
const {plan,readCurrent,writePlan}=require('../src/services/academicTemplates');
...
(async()=>{const c=await getClient();try{
 ...
 await c.query('LOCK TABLE institutions,faculties,schools,departments,courses,academic_template_assignments IN SHARE ROW EXCLUSIVE MODE');
 ...
 const stamp=Date.now(),report=path.join(dir,`shared-academics-${stamp}-${apply?'apply':'dry-run'}.json`),backup=path.join(dir,`shared-academics-before-${stamp}.json`);
 ...
 console.log(`${apply?'Committed':'Preview only'}: ${result.assignments.length} institutions, ${result.faculties.length} faculties, ${result.schools.length} schools, ${result.departments.length} departments, ${result.courses.length} courses.`);
 console.log(`${result.skipped.length} institutions skipped; ${result.conflicts.length} matching conflicts left for frontend review.`);
