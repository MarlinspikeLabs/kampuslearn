...
(async()=>{const c=await getClient();try{
 await c.query('BEGIN');
 await c.query(fs.readFileSync(path.resolve(__dirname,'../migrations/20260910_course_defaults.sql'),'utf8'));
 await c.query('COMMIT');console.log('Course default tracking is ready. No courses seeded.');
