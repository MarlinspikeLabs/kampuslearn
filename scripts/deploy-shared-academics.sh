...
test "$(git rev-parse HEAD)" = "$(git rev-parse '83abe0c2e9ba7ec2cbedaa5e995d5488f35ca48a^{commit}')"
...
git cat-file -e "$target:backend/src/services/academicTemplates.js"
...
backup="$HOME/kampuslearn-backups/shared-academics-$stamp"
mkdir -p "$backup"
...
rollback(){
  ...
  echo 'The additive seed-tracking table may remain; existing academic records were not reseeded.'
  exit 1
}
trap rollback ERR
pm2 stop kampuslearn-web
...
git switch -c "deploy/shared-academics-$stamp" "$target"

node backend/scripts/migrate-shared-academics.cjs
...
trap - ERR
echo 'Shared defaults deployed. New institutions now receive editable academic structures.'
echo 'For existing institutions, run backend/scripts/seed-shared-academics.cjs --dry-run, then --apply.'
