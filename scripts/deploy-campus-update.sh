#!/usr/bin/env bash
# Run on the VPS from the preparation worktree after fetching the campus update release.
set -Eeuo pipefail
umask 077
exec 9>"$HOME/kampuslearn-deploy.lock"
flock -n 9 || { echo 'Another deployment is running. Read its log before starting again.'; exit 1; }
trap 'echo "Preflight failed at line $LINENO. No deployment changes have started." >&2' ERR
prep_root=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)
cd /var/www/kampuslearn
test -z "$(git status --porcelain)"
test "$(git rev-parse HEAD)" = "$(git rev-parse '776d438^{commit}')"
target=$(git rev-parse "${1:-origin/feat/student-dashboard-redesign}^{commit}")
git merge-base --is-ancestor HEAD "$target"
git merge-base --is-ancestor 4bf18c4 "$target"
git cat-file -e "$target:backend/src/routes/admin_console.js"
git cat-file -e "$target:backend/migrations/20260908_content_scope.sql"
...
backup="$HOME/kampuslearn-backups/campus-update-$stamp"
mkdir -p "$backup"
...
rollback(){
  ...
  echo 'The content scope schema is retained; existing and shared content records are preserved.'
  exit 1
}
trap rollback ERR
pm2 stop kampuslearn-web
...
git switch -c "deploy/campus-update-$stamp" "$target"
node backend/scripts/migrate-content-scope.cjs
node backend/scripts/migrate-marketplace.cjs
(
  cd frontend
...
trap - ERR
echo 'Generic uploads and pre-launch Marketplace deployed.'
echo 'Academic import runs separately using import-academic-reference.cjs --dry-run, then --apply.'
